#include "Framework.h"
#include "Engine/Services/Compatibility/PsyCross/Render.h"

#include "Engine/Services/Compatibility/PsyCross/Gpu.h"
...
#define PSX_SCREEN_ASPECT      (240.0f / 320.0f) // PSX screen maps to this aspect.
...
typedef struct
{
    ...
    int    width;
    int    height;
    int    nbytes; // Byte count in PBO buffer.
    uchar* pixels; // Downloaded pixels.
} GrPBO;
...
int PBO_Init(GrPBO* pbo, GLenum format, int w, int h, int num)
{
    ...
    pbo->pixels = (uchar*)malloc(pbo->nbytes);
...
void PBO_Download(GrPBO* pbo)
{
    ...
    else
    {
        ...
        ptr = (uchar*)glMapBuffer(GL_PIXEL_PACK_BUFFER, GL_READ_ONLY);
        if (NULL != ptr)
        {
...
GLint uprojection3DLoc;
GLint ubilinearFilterLoc;
GLint utexelSizeLoc;
...
void GR_Perspective3D(const float fov, const float width, const float height, const float zNear, const float zFar)
{
    ...
    glUniformMatrix4fv(uprojection3DLoc, 1, GL_FALSE, persp);
}
...
void PsyX_GetPSXWidescreenMappedViewport(struct RECT16* rect)
{
#if USE_PGXP
...
void GR_SetTexture(TextureID texture, TexFormat texFormat)
{
    ...
    case TF_4_BIT:
        ...
        ubilinearFilterLoc = g_gte_shader_4.bilinearFilterLoc;
        uprojectionLoc = g_gte_shader_4.projectionLoc;
        uprojection3DLoc = g_gte_shader_4.projection3DLoc;
        utexelSizeLoc = -1;
        break;

    case TF_8_BIT:
        GR_SetShader(g_gte_shader_8.shader);
        ubilinearFilterLoc = g_gte_shader_8.bilinearFilterLoc;
        uprojectionLoc = g_gte_shader_8.projectionLoc;
        uprojection3DLoc = g_gte_shader_8.projection3DLoc;
        utexelSizeLoc = -1;
        break;

    case TF_16_BIT:
        GR_SetShader(g_gte_shader_16.shader);
        ubilinearFilterLoc = g_gte_shader_16.bilinearFilterLoc;
        uprojectionLoc = g_gte_shader_16.projectionLoc;
        uprojection3DLoc = g_gte_shader_16.projection3DLoc;
        utexelSizeLoc = -1;
        break;

    case TF_32_BIT_RGBA:
        GR_SetShader(g_gte_shader_32_rgba.shader);
        ubilinearFilterLoc = g_gte_shader_32_rgba.bilinearFilterLoc;
        uprojectionLoc = g_gte_shader_32_rgba.projectionLoc;
        uprojection3DLoc = g_gte_shader_32_rgba.projection3DLoc;
        utexelSizeLoc = g_gte_shader_32_rgba.texelSizeLoc;
        break;
    }
    ...
    glUniform1i(ubilinearFilterLoc, g_cfg_bilinearFiltering);
...
{
    glUniform2f(utexelSizeLoc, 1.0f / (float)width, 1.0f / (float)height);
}
...
void GR_ClearVRAM(int x, int y, int w, int h, uchar r, uchar g, uchar b)
{
    ...
    ushort* dst = vram + x + y * VRAM_WIDTH;
...
void GR_SaveVRAM(const char* outputFileName, int x, int y, int width, int height, int bReadFromFrameBuffer)
{
    ...
    fwrite(TGAheader, sizeof(uchar), 12, fp);
    fwrite(header, sizeof(uchar), 6, fp);
...
void GR_CopyRGBAFramebufferToVRAM(uint* src, int x, int y, int w, int h, int update_vram, int flip_y)
{
    assert(x >= 0);
    assert(y >= 0);
...
void GR_ReadFramebufferDataToVRAM()
{
    ...
    if(g_glFramebufferPBO.pixels)
    {
        ...
        glBindTexture(GL_TEXTURE_2D, 0);
        GR_CopyRGBAFramebufferToVRAM((uint*)g_glFramebufferPBO.pixels, x, y, w, h, 0, 0);
    }
#endif
}
...
void GR_SetOffscreenState(const RECT16* offscreenRect, int enable)
{
    ...
    else
    {
        ...
        // copy rendering results to VRAM texture
        {
            ...
            GR_CopyRGBAFramebufferToVRAM((uint*)g_glOffscreenPBO.pixels, 
