...
#define GTE_SF(op)    ((op >> 19) & 0x1)
#define GTE_MX(op)    ((op >> 17) & 0x3)
#define GTE_V(op)     ((op >> 15) & 0x3)
#define GTE_CV(op)    ((op >> 13) & 0x3)
#define GTE_LM(op)    ((op >> 10) & 0x1)
...
int LIM(int value, int max, int min, uint flag)
{
    if (value > max)
    {
        C2_FLAG |= flag;
...
uint gte_divide(ushort numerator, ushort denominator)
{
    if (numerator < (denominator * 2))
    {
        ...
        uint reciprocal = ((r2 * r3) + 0x80) >> 8;

        return (uint)((((unsigned long long)reciprocal * (numerator << shift)) + 0x8000) >> 16);
    }
