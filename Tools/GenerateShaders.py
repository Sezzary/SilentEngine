...
def generate_shaders():
    ...
    try:
        ...
        # Define output formats based on input OS.
        if len(sys.argv) > 1:
            ...
            if build_os == "windows":
                ...
                raise Exception(f"Passed invalid OS argument `{build_os}`.")
        else:
            raise Exception("No OS argument passed.")
        ...
        for shader_source in shader_sources:
            for format in formats:
                ...
                make_new_build = False
                if os.path.isfile(shader_output):
                    if os.path.getmtime(shader_source) > os.path.getmtime(shader_output):
                        make_new_build = True
                else:
                    make_new_build = True
                ...
                if make_new_build:
                    command = [shadercross_exe, shader_source, "-o", shader_output]
                    result  = subprocess.run(command, capture_output=True)
                    if result.returncode == 0:
                        build_count += 1
                    else:
                        print(f"Error running `{SHADERCROSS_NAME}` for `{name}`: {result.stderr.decode()}")
                        fail_names.append(output_name)

        # @todo Failed shaders are built anyway. Implement temporary file handling to validate them before copying them into the build.

        # Report status.
        if build_count == 0 and len(fail_names) == 0:
            print("Shaders are already up-to-date.")
        else:
            print(f"Built {build_count} shader{'' if build_count == 1 else 's'}.")

            if (len(fail_names) > 0):
                print(f"The following failed to build (see error details above):")
