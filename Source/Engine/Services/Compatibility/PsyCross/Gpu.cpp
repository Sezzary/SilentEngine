...
void LineSwapSourceVerts(VERTTYPE*& p0, VERTTYPE*& p1, uchar*& c0, uchar*& c1)
{
...
void MakeTexcoordQuad(GrVertex* vertex, uchar* uv0, uchar* uv1, uchar* uv2, uchar* uv3, short page, short clut, uchar dither)
{
    assert(uv0);
    assert(uv1);
    assert(uv2);
...
void MakeTexcoordTriangle(GrVertex* vertex, uchar* uv0, uchar* uv1, uchar* uv2, short page, short clut, uchar dither)
{
    assert(uv0);
    assert(uv1);
    assert(uv2);
...
void MakeTexcoordRect(GrVertex* vertex, uchar* uv, short page, short clut, short w, short h)
{
    assert(uv);
...
void MakeTexcoordLineZero(GrVertex* vertex, uchar dither)
{
    const uchar bright = 1;
...
void MakeTexcoordTriangleZero(GrVertex* vertex, uchar dither)
{
    const uchar bright = 1;
...
void MakeTexcoordQuadZero(GrVertex* vertex, uchar dither)
{
    const uchar bright = 1;
...
void MakeColourLine(GrVertex* vertex, bool shadeTexOn, uchar* col0, uchar* col1)
{
    if (!shadeTexOn)
    {
...
void MakeColourTriangle(GrVertex* vertex, bool shadeTexOn, uchar* col0, uchar* col1, uchar* col2)
{
    if (!shadeTexOn)
    {
...
void MakeColourQuad(GrVertex* vertex, bool shadeTexOn, uchar* col0, uchar* col1, uchar* col2, uchar* col3)
{
    if (!shadeTexOn)
    {
        MakeColourNoShade(vertex, 4);
...
void DrawAllSplits()
{
    ...
    if (g_dbg_emulatorPaused)
    {
        for (int i = 0; i < 3; i++)
        {
            auto* vert = &g_vertexBuffer[g_dbg_polygonSelected + i];
            vert->r    = 255;
            vert->g    = 0;
            vert->b    = 0;
...
void ParsePrimitivesLinkedList(uint* p, int singlePrimitive)
{
    ...
    // Set up single primitive flag (needed for `AddSplits`).
    ...
    {
        // Run through `OT_TAG` linked list.
        for (uintptr_t basePacket = reinterpret_cast<uintptr_t>(p);; basePacket = reinterpret_cast<uintptr_t>(nextPrim(basePacket)))
        {
            ...
            if (tagLength > 0)
            {
                ...
                const uintptr_t endPacket = basePacket + (tagLength + P_LEN) * sizeof(uint);
                int primLength = 0;
                while (currentPacket < endPacket)
                {
                    primLength = ParsePrimitive(reinterpret_cast<P_TAG*>(currentPacket));
                    currentPacket += (primLength + P_LEN) * sizeof(uint);
                }
...
{
#if USE_PGXP && USE_EXTENDED_PRIM_POINTERS
    const ushort gteIndex = polyTag->pgxp_index;
#else
    const ushort gteIndex = 0xFFFF;
#endif
...
{
    const uint* codePtr        = (uint*)&polyTag->pad0;
    int         processedLongs = 0;
    for (int i = 0; i < polyTag->len; ++i)
    {
        const uint code        = codePtr[i];
...
static int ProcessPsyXPrims(P_TAG* polyTag)
{
    ...
    case 0x02:
    {
        // [A] PsyCross custom texture packet.
